    ...
    return (
        <div style={{ maxWidth: 600, margin: '2rem auto', background: '#fff', borderRadius: 16, boxShadow: '0 2px 12px rgba(34,128,224,0.09)', padding: '2.5rem 2rem', fontFamily: 'Segoe UI, Arial, sans-serif' }}>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div>
                    <button type="button" onClick={() => toggle('transfer')} style={sectionBtnStyle}>
                        <span>Change Transfer Password</span>
                        <span style={{ transform: open.transfer ? 'rotate(90deg)' : 'none', transition: '0.2s' }}>{'>'}</span>
                    </button>
                    {open.transfer && (
                        <div style={dropdownStyle}>
                            <input type="password" name="transferPassword" value={settings.transferPassword} onChange={handleChange} style={inputStyle} />
                            <button type="button" onClick={(e) => handleSave(e, 'transfer password')} style={saveBtnStyle}>Save Transfer Password</button>
                        </div>
                    )}
                </div>
                {/* Security Question */}
                <div>
                    <button type="button" onClick={() => toggle('security')} style={sectionBtnStyle}>
                        <span>Security Question</span>
                        <span style={{ transform: open.security ? 'rotate(90deg)' : 'none', transition: '0.2s' }}>{'>'}</span>
                    </button>
                    {open.security && (
                        <div style={dropdownStyle}>
                            <input name="securityQuestion" value={settings.securityQuestion} onChange={handleChange} style={inputStyle} placeholder="e.g. Your first pet's name?" />
                            <input name="securityAnswer" value={settings.securityAnswer} onChange={handleChange} style={inputStyle} placeholder="Answer" />
                            <button type="button" onClick={(e) => handleSave(e, 'security question')} style={saveBtnStyle}>Save Security Question</button>
                        </div>
                    )}
                </div>
                {/* Savings Setting */}
                <div>
                    <button type="button" onClick={() => toggle('savings')} style={sectionBtnStyle}>
                        <span>Savings Setting</span>
                        <span style={{ transform: open.savings ? 'rotate(90deg)' : 'none', transition: '0.2s' }}>{'>'}</span>
                    </button>
                    {open.savings && (
                        <div style={dropdownStyle}>
                            <label>
                                <input type="checkbox" name="savingsEnabled" checked={settings.savingsEnabled} onChange={handleChange} /> Enable Savings
                            </label>
                            <button type="button" onClick={(e) => handleSave(e, 'savings setting')} style={saveBtnStyle}>Save Savings Setting</button>
                        </div>
                    )}
                </div>
                {/* Feedback */}
                <div>
                    <button type="button" onClick={() => toggle('feedback')} style={sectionBtnStyle}>
                        <span>Feedback</span>
                        <span style={{ transform: open.feedback ? 'rotate(90deg)' : 'none', transition: '0.2s' }}>{'>'}</span>
                    </button>
                    {open.feedback && (
                        <div style={dropdownStyle}>
                            <textarea name="feedback" value={settings.feedback} onChange={handleChange} style={inputStyle} placeholder="Share your feedback..." />
                            <button type="button" onClick={(e) => handleSave(e, 'feedback')} style={saveBtnStyle}>Save Feedback</button>
                        </div>
                    )}
                </div>
                {/* Suggestions */}
                <div>
                    <button type="button" onClick={() => toggle('suggestions')} style={sectionBtnStyle}>
                        <span>Suggestions</span>
                        <span style={{ transform: open.suggestions ? 'rotate(90deg)' : 'none', transition: '0.2s' }}>{'>'}</span>
                    </button>
                    {open.suggestions && (
                        <div style={dropdownStyle}>
                            <textarea name="suggestions" value={settings.suggestions} onChange={handleChange} style={inputStyle} placeholder="Any suggestions?" />
                            <button type="button" onClick={(e) => handleSave(e, 'suggestions')} style={saveBtnStyle}>Save Suggestions</button>
                        </div>
                    )}
                </div>
            ...
            {/* Close Account */}
            <div>
                <button type="button" onClick={() => toggle('close')} style={sectionBtnStyle}>
                    <span>Close Account</span>
                    <span style={{ transform: open.close ? 'rotate(90deg)' : 'none', transition: '0.2s' }}>{'>'}</span>
                </button>
                {open.close && (
                    <div style={dropdownStyle}>
                        <button onClick={handleCloseAccount} style={{ background: '#e74c3c', color: '#fff', border: 'none', borderRadius: 8, padding: '0.7rem 1.5rem', fontWeight: 600, fontSize: '1rem' }}>Confirm Close Account</button>
                        {closeAccount && (
                            <div style={{ marginTop: 20, color: '#e74c3c', fontWeight: 600 }}>
